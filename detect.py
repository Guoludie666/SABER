...
def main():
    ...
    #mix_texts = read_data__('ad_data/christianity.txt','ad_data/islam.txt','ad_data/judaism.txt')
    #mix_texts=read_data_csv('entailment_data.csv')
    mix_texts = read_data_('race/generate_race1.txt','race/generate_race2.txt')

    all_texts = mix_texts[:]

    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_and_save_data, all_texts[i:i+3], comprehend, 'auto_mix_race_responses.json') for i in range(0, len(all_texts), 3)]
        results = []
